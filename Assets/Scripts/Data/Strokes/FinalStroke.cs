...
public class FinalStroke : Stroke
{
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            //Segment s = segments.First.Value;
            INode[] endpoints = new INode[] { s.GetStartNode(), s.GetEndNode() };
            float[] onCurveParams = new float[] { s.GetStartParam(), s.GetEndParam() };
            //for (int i = 0; i < 2; i++)
            //{
            //    Vector3 n_transported = ParallelTransport(endpoints[i].Normal, onCurveParams[i], onCurveParams[(i + 1) % 2]);
            //    Vector3 dir = this.transform.TransformDirection(n_transported);
            //    Vector3 nodePos = this.transform.TransformPoint(endpoints[(i + 1) % 2].Position);
            //    Vector3 to = nodePos + 0.08f * dir;
            //    Handles.color = Color.white;
            //    Handles.DrawAAPolyLine(5f, new Vector3[] { nodePos, to });
            //}


            if (DebugGizmos)
            {
                // Draw parallel transported normal from start node to end node
                if (endpoints[0].IncidentCount > 2)
                {
                    // normal at start node is well defined
                    // transport it to end node and draw it
                    Vector3 n_transported = ParallelTransport(endpoints[0].Normal, onCurveParams[0], onCurveParams[1]);
                    Vector3 dir = this.transform.TransformDirection(n_transported);
                    Vector3 nodePos = this.transform.TransformPoint(Curve.GetPoint(onCurveParams[1] - 0.1f));
                    Vector3 to = nodePos + 0.08f * dir;
                    Handles.color = Color.white;
                    Handles.DrawAAPolyLine(5f, new Vector3[] { nodePos, to });
                }
                ...
                {
                    // normal at start node is not well defined
                    // walk along graph until a node has a well defined normal (3 neighbors or more)
                    ISegment currentSegment = s;
                    INode currentNode = endpoints[0];
                    while (true)
                    {
                        if (currentNode.IncidentCount < 2)
                            break;
                        if (currentNode.Equals(endpoints[1]))
                            break;

                        currentSegment = currentNode.GetNext(currentSegment);
                        currentNode = currentSegment.GetOpposite(currentNode);

                        if (currentNode.IncidentCount > 2)
                        {
                            // Transport this normal all the way back to the end node of current segment
                            // And display it

                            Vector3 currentNormal = currentNode.Normal;

                            while (!currentNode.Equals(endpoints[0]))
                            {
                                // transport normal along segment
                                INode nextNode = currentSegment.GetOpposite(currentNode);
                                ISegment nextSegment = nextNode.GetNext(currentSegment);
                                currentNormal = currentSegment.Transport(currentNormal, nextNode);

                                // Parallel transport between segments
                                // (works the same as for parallel transport along a curve,
                                // we transport "across the node" by taking tangents from the 2 curves)

                                Vector3 prev_tangent = -currentSegment.GetTangentAt(nextNode);
                                Vector3 tangent = nextSegment.GetTangentAt(nextNode);
                                ...
                                if (axis.magnitude > float.Epsilon)
                                {
                                    ...
                                    currentNormal = Quaternion.AngleAxis(theta * Mathf.Rad2Deg, axis) * currentNormal;
                                }

                                Vector3 dir2 = this.transform.TransformDirection(currentNormal);
                                Vector3 nodePos2 = this.transform.TransformPoint(nextNode.Position);
                                Vector3 to2 = nodePos2 + 0.08f * dir2;
                                Handles.color = Color.cyan;
                                Handles.DrawAAPolyLine(5f, new Vector3[] { nodePos2, to2 });


                                // travel back until the end node of current segment
                                currentNode = nextNode;
                                currentSegment = nextSegment;
                            }

                            currentNormal = currentSegment.Transport(currentNormal, currentSegment.GetOpposite(currentNode));

                            // Finally, draw transported normal
                            Vector3 dir = this.transform.TransformDirection(currentNormal);
                            Vector3 nodePos = this.transform.TransformPoint(Curve.GetPoint(onCurveParams[1] - 0.1f));
                            Vector3 to = nodePos + 0.08f * dir;

...
* Clone this repository. (Warning: downloading the repository from gitlab UI instead of cloning leads to the files stored with git LFS to be missing, which will cause build problems.) If you are encountering build errors related to SteamVR missing an assembly, it might be because you are missing the files stored with git LFS, you can re-try fetching them.
